...
/// 手柄类型
public enum GamePadCategory: Int {
    case Others     // 未知
    case XBox       // Xbox
    case DualShock  // PS4
    case DualSense  // PS5
...
public enum GamePadInputDataType {
...
/// 单击类型按钮
public typealias GamePadButtonCallBack = (_ model: GamePadModel, _ keyName: GamePadButton, _ pressed: Bool) -> Void

/// 力度类型按钮
public typealias GamePadIntensityCallBack = (_ model: GamePadModel, _ keyName: GamePadButton, _ intensity: Float) -> Void

/// 二维数据类型按钮
public typealias GamePadDimensionCallBack = (_ model: GamePadModel, _ keyName: GamePadButton, _ point: CGPoint) -> Void

/// 手柄数据监听回调（将三种数据类型合并成统一接口）
public typealias GamePadInputCallBack = (_ model: GamePadModel, _ button: GamePadButton, _ inputValue: GamePadInputDataType) -> Void
